...
このアセットでは以下のスクリプトからビルドしたFFmpeg本体(Windows/Mac/Linux)、もしくはプラグイン(Android/iOS)を使用しております。
Windowsの場合、以下のURLからバイナリ本体をインポートするか、ビルドしてください。

https://github.com/NON906/ffmpeg-windows-build-helpers/releases/

他のバイナリも、外部ライブラリは一部のみ（「ライセンス」を参照）にしておりますので、使用したい機能がない場合は手順に従って再ビルドし、それに置き換えてください。
